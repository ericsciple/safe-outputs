...
| `footer` / `messages.*` | **Footer** = an attribution line auto-appended to every issue/PR/comment body (default `> Generated by [Workflow](run_url) for #42`). **`messages.*`** = 19 customizable templates (footer text, a "disclosure header", status messages). `footer: false` hides the visible line. | Traceability/attribution, not security. **Decision: low-priority candidate** — lean toward an optional minimal `> Generated by <workflow> (<run-url>)`; default on/off TBD. |
...
1. **`max` — adopt gh-aw semantics (call-count), retire ours (per-call cap).**
   ...
   - **Tracking mechanism (our stateless-per-call model).** gh-aw uses an in-memory counter in a
     **long-lived** MCP server; we spawn a **fresh `safe-outputs <op>` process per call**, so we need
     ...
     - The harness passes each configured MCP server a stable **`--id <name>`** (the `mcp-config` key)
       and a **per-run state dir** (e.g. under `RUNNER_TEMP`).
     - On each **successful** apply, append one line to `${STATE_DIR}/${id}.calls`; before applying,
       if the line count ≥ `--max`, **reject** with an actionable error. Only successes count (a
       validation-rejected call doesn't burn the budget).
     - Small POSIX appends are atomic → **lock-free** even if tool calls overlap; line-count = call
       count.
     - **Per-instance scoping falls out for free:** `mcp-config: { add_labels1: …, add_labels2: … }`
       → different `--id` → different files → **counted separately** (the requirement). State dir is
       per-run, so counts reset each run. No `--max` → no file, no cap.
...
6. **`footer` / `messages.*` — low-priority candidate.** Optional minimal attribution footer
   (`> Generated by <workflow> (<run-url>)`) for traceability; default on/off TBD. The full
   `messages.*` template system is out of scope for now.
